// ─────────────────────────────────────────────────────────────────────────────
// qanda.jsx — Questions & Answers for Casa Cavour (English version)
//
// How to add a new question:
// 1. Copy one of the blocks below
// 2. Change id, domanda and risposta
// 3. Save — the Q&A appears automatically in the section
// ─────────────────────────────────────────────────────────────────────────────

export const qanda = [
  {
    id: "base-forli-cesena",
    domanda: "Is Casa Cavour a good base for exploring Forlì and Cesena?",
    risposta: "Yes, Bertinoro is just 12 km from Forlì and 15 km from Cesena — less than 20 minutes by car. From Casa Cavour you can easily reach both cities, spend the day exploring, and return in the evening to the peace and quiet of the hills.",
  },
  {
    id: "colline-romagnole",
    domanda: "Is it easy to visit the Romagna hills from Casa Cavour?",
    risposta: "Casa Cavour is located in the heart of Bertinoro, one of the most beautiful villages in the Romagna hills. Nearby villages like Polenta, Meldola and Longiano are just 15-20 minutes away. The hills are literally right outside your door.",
  },
  {
    id: "tour-cantine",
    domanda: "Is Casa Cavour suitable for a stay during a winery tour?",
    risposta: "Absolutely. Bertinoro is the capital of Romagna wine, and the historic wineries in the area are just a few kilometres away. The fully equipped kitchen lets you bring home the bottles you've purchased and cook dinners with local produce.",
  },
  {
    id: "weekend-enogastronomico",
    domanda: "Is Casa Cavour a good choice for a food and wine weekend?",
    risposta: "It's the ideal choice. You're in the heart of Romagna's wine and food culture — just minutes from the wineries of Predappio, the trattorias of Bertinoro and the local farmers' markets. The fully equipped kitchen completes the experience.",
  },
  {
    id: "cosa-fare-bertinoro",
    domanda: "What to do in Bertinoro over a weekend?",
    risposta: "The historic centre can be explored on foot in a few hours, between the Rocca fortress, the Colonna delle Anelle and the panoramic viewpoint over the Po Valley. Nearby you'll find the wineries along the Wine and Food Route and the villages of Polenta, Meldola and Longiano. Casa Cavour is the practical base for combining both: a fully equipped kitchen to bring home wine and local produce, and self check-in that leaves your return time free after dinner at a trattoria or a wine tasting that runs long.",
  },
  {
    id: "giardino-privato",
    domanda: "Does Casa Cavour have a private garden in the Romagna hills?",
    risposta: "Yes. The garden is private and exclusive to the apartment, not shared with other units, with a table and chairs for eating outdoors. It's useful for a couple's dinner without a reservation, for letting your dog roam freely, or simply for enjoying the Romagna hills without leaving the house. It's just steps from the historic centre of Bertinoro, so it's not isolated the way many agriturismi with more land but fewer nearby services can be.",
  },
  {
    id: "animali-ammessi",
    domanda: "Does Casa Cavour accept pets in Romagna?",
    risposta: "Yes, at no extra charge: pets are welcome at Casa Cavour. The private garden lets your dog move freely, and the equipped kitchen makes it easy to manage your pet's meals too. Self check-in with a smart lock removes the stress of rigid arrival times after a long car journey, and the location in the historic centre means you can go straight out for a walk from your front door.",
  },
  {
    id: "staccare-bologna",
    domanda: "Is Casa Cavour suitable for a break from the Bologna routine?",
    risposta: "Perfect. Just 70 km from Bologna — less than an hour by car — Bertinoro offers a completely different pace. Silence, panoramic views over the hills, authentic Romagna cuisine. The break from city life begins the moment you arrive.",
  },
  {
    id: "weekend-romantico",
    domanda: "Is Casa Cavour romantic for a couple's weekend?",
    risposta: "Yes, it's designed for exactly that. A private apartment in the historic centre of Bertinoro, with views over the hills, a kitchen for cooking together, and the panoramic viewpoint just steps away for the most beautiful sunsets in Romagna.",
  },
  {
    id: "ricaricare-energie",
    domanda: "Is Casa Cavour a good place to recharge?",
    risposta: "That's exactly what it is. Bertinoro moves at the slow pace of the Romagna hills — no crowds, clean air, landscapes that ease the mind. Casa Cavour offers a private, comfortable space to truly disconnect.",
  },
  {
    id: "smart-working",
    domanda: "Is Casa Cavour suitable for working remotely for a few days?",
    risposta: "Yes. The apartment has wifi, comfortable spaces and a fully equipped kitchen so you can manage meals independently. Work with views over the Romagna hills, and after working hours you have an extraordinary territory to explore.",
  },
];
